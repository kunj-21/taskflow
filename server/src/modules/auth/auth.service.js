...
import { acceptInvitation, createOrganization, listMyOrganizations } from '../orgs/orgs.service.js';
...
export async function issueSession(user) {
  ...
  return { accessToken, refresh, user: publicUser(user), organizations: await listMyOrganizations(user.id) };
}
...
// Sign-up either joins an existing org through an invitation, or creates a new org the user owns.
// Both happen in one transaction so a failed invite never leaves an orphaned account.
export async function register({ email, name, password, orgName, inviteToken }) {
  const existing = await prisma.user.findUnique({ where: { email } });
  if (existing) throw ApiError.conflict('Email already registered');
  if (!inviteToken && !orgName) throw ApiError.badRequest('Organization name is required', { orgName: ['Required'] });

  const passwordHash = await bcrypt.hash(password, 12);
  const { user, joined } = await prisma.$transaction(async (tx) => {
    const user = await tx.user.create({ data: { email, name, passwordHash } });
    if (inviteToken) return { user, joined: await acceptInvitation(inviteToken, user, tx) };
    return { user, joined: await createOrganization(user.id, orgName, tx) };
  });
  return { ...(await issueSession(user)), joined };
}
...
export async function upsertGoogleUser(profile) {
  ...
  // New Google users start with no organization; the app then offers to create one or accept an invite.
  return prisma.user.create({ data: { email, name: profile.displayName || email, googleId: profile.id, avatarUrl } });
}
